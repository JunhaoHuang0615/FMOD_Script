...
studio.menu.addMenuItem({
  ...
  function checkEvent() {
    ...
    studio.ui.showModalDialog({
        ...
        items: [
            { widgetType: studio.ui.widgetType.Label, text: "Select only Events in Event Editor at first" },
            // {
            //     widgetType: studio.ui.widgetType.ComboBox,
            //     widgetId: "m_targetBox",
            //     items: bank_dict,
            //     currentIndex: 0,
            // },
            { widgetType: studio.ui.widgetType.PushButton, 
                text: "check selected Events", 
                onClicked: function() 
                    {   
                        selectedEvents = []
                        var tempselectEvent = studio.window.browserSelection()
                        tempselectEvent.forEach(function(element) {
                            selectedEvents.push(element.getPath())
                        });
                        var alert_text = ""
                        selectedEvents.forEach(function(element){
                            alert_text = alert_text + element + "\n"
                        })
                        alert(alert_text);
                    } 
            },
            ...
            { widgetType: studio.ui.widgetType.PushButton, 
                text: "check In {HotFix} only or not", 
                onClicked: function() 
                    {   
                        //获得所有的选取事件
                        selectedEvents = []
                        var tempselectEvent = studio.window.browserSelection()
                        tempselectEvent.forEach(function(element) {
                            if(element.getPath().indexOf("event") > -1){
                        ...
                        if(alert_text === ""){
                            alert("Selected event has been in correct Bank");
                        }
                        else{
                            //绘制事件名称以及要加入的Bank
                            //打开添加事件的界面：========================================================================
